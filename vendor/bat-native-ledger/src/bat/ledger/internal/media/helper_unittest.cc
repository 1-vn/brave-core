...
// npm run test -- onevn_unit_tests --filter=MediaHelperTest.*
...
TEST(MediaHelperTest, GetMediaKey) {
  // provider is missing
  std::string result = onevnledger_media::GetMediaKey("key", "");
  ASSERT_EQ(result, "");

  // key is missing
  result = onevnledger_media::GetMediaKey("", "youtube");
  ASSERT_EQ(result, "");

  // all ok
  result = onevnledger_media::GetMediaKey("key", "youtube");
...
TEST(MediaHelperTest, GetTwitchParts) {
  ...
  // string is empty
  onevnledger_media::GetTwitchParts("", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);

  // random string
  onevnledger_media::GetTwitchParts("this is random string", &twitch_parts);
  ASSERT_EQ(twitch_parts.size(), 0u);
  ...
  // all ok
  onevnledger_media::GetTwitchParts(post_data, &twitch_parts);
...
//  // string empty
  std::string result = onevnledger_media::ExtractData("", "/", "!");
  ASSERT_EQ(result, "");

  // missing start
  result = onevnledger_media::ExtractData("st/find/me!", "", "!");
