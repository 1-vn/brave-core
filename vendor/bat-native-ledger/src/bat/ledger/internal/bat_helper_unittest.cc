...
// npm run test -- onevn_unit_tests --filter=BatHelperTest.*

TEST(BatHelperTest, isProbiValid) {
  // zero probi
  bool result = onevnledger_bat_helper::isProbiValid("0");
  ASSERT_EQ(result, true);

  // 1 BAT
  result = onevnledger_bat_helper::isProbiValid("1000000000000000000");
  ASSERT_EQ(result, true);
  ...
  // not correct probi
  result = onevnledger_bat_helper::isProbiValid("10-00000000000000000");
  ASSERT_EQ(result, false);

  // not correct probi
  result = onevnledger_bat_helper::isProbiValid("fds000000000");
  ASSERT_EQ(result, false);

  // not correct probi
  result = onevnledger_bat_helper::isProbiValid(
...
TEST(BatHelperTest, HasSameDomainAndPath) {
  ...
  // regular url
  bool result = onevnledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
  ASSERT_EQ(result, true);

  // empty url with portion
  url = std::string();
  result = onevnledger_bat_helper::HasSameDomainAndPath(
      url, url_portion, path);
  ASSERT_EQ(result, false);
