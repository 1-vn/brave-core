...
export const getShieldSettingsForTabData = (tabData?: chrome.tabs.Tab) => {
  ...
  return Promise.all([
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_ONEVN_SHIELDS } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_ADS } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_TRACKERS } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_HTTP_UPGRADABLE_RESOURCES } }),
    chrome.onevnShields.javascript.getAsync({ primaryUrl: origin }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_FINGERPRINTING } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, secondaryUrl: 'https://firstParty/*', resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_FINGERPRINTING } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_COOKIES } }),
    chrome.onevnShields.plugins.getAsync({ primaryUrl: origin, secondaryUrl: 'https://firstParty/', resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_COOKIES } })
  ]).then((details) => {
    ...
    const onevnShields = isHttpOrHttps(origin) ? details[0].setting : 'block'
    return {
      url: url.href,
      origin,
      hostname,
...
/**
 * Changes the onevn shields setting at origin to be allowed or blocked.
...
export const setAllowHTTPUpgradableResources = (origin: string, setting: BlockOptions) => {
  ...
  return chrome.onevnShields.plugins.setAsync({
    primaryPattern,
...
export const setAllowJavaScript = (origin: string, setting: string) =>
  chrome.onevnShields.javascript.setAsync({
    primaryPattern: origin + '/*',
    setting,
    scope: getScope()
  })
...
export const setAllowFingerprinting = (origin: string, setting: string) => {
  ...
  const p1 = chrome.onevnShields.plugins.setAsync({
    primaryPattern: origin + '/*',
  ...
  const p2 = chrome.onevnShields.plugins.setAsync({
    primaryPattern: origin + '/*',
...
export const setAllowCookies = (origin: string, setting: string) => {
  ...
  const p3 = chrome.onevnShields.plugins.setAsync({
    primaryPattern: origin + '/*',
    secondaryPattern: 'https://firstParty/*',
    resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_COOKIES },
    setting: firstPartySetting,
...
export const setAllowScriptOriginsOnce = (origins: Array<string>, tabId: number) =>
  new Promise<void>((resolve) => {
    chrome.onevnShields.allowScriptsOnce(origins, tabId, () => {
      resolve()
    })
  })
