...
class OnevnBookmarkChangeProcessorTest : public testing::Test {
 ...
 protected:
  void SetUp() override {
    ...
    sync_prefs_.reset(new onevn_sync::prefs::Prefs(profile_->GetPrefs()));

    change_processor_.reset(onevn_sync::BookmarkChangeProcessor::Create(
        profile_.get(),
        sync_client(),
        sync_prefs_.get() ));
  ...
  onevn_sync::BookmarkChangeProcessor* change_processor() {
...
TEST_F(OnevnBookmarkChangeProcessorTest, GetAllSyncData) {
  ...
  onevn_sync::SyncRecordAndExistingList records_and_existing_objects;
...
TEST_F(OnevnBookmarkChangeProcessorTest, MigrateOrdersForPermanentNodes) {
  ...
  using onevn_sync::BookmarkChangeProcessor;
