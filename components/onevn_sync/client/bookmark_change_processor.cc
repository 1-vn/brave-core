...
class ScopedPauseObserver {
 public:
  explicit ScopedPauseObserver(onevn_sync::BookmarkChangeProcessor* processor) :
 ...
 private:
  onevn_sync::BookmarkChangeProcessor* processor_;  // Not owned
};
...
uint64_t GetIndexByOrder(const bookmarks::BookmarkNode* root_node,
  ...
  while (index < root_node->child_count()) {
    ...
    if (!node_order.empty() &&
        onevn_sync::CompareOrder(record_order, node_order))
      return index;
...
void BookmarkChangeProcessor::BookmarkNodeMoved(BookmarkModel* model,
  ...
  // clearing. Also https://github.com/1-vn/sync/issues/231 blocks update to
  // another devices
