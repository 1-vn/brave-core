...
chrome.onevnSync.onGotInitData.addListener(function(seed, device_id, config, sync_words) {
  ...
  if (sync_words) {
    try {
      ...
      chrome.onevnSync.syncSetupError('ERR_SYNC_WRONG_WORDS')
      return;
    }
  }
...
chrome.onevnSync.onFetchSyncRecords.addListener(function(category_names, start_at, max_records) {
...
chrome.onevnSync.onFetchSyncDevices.addListener(function() {
  console.log("fetch-sync-devices");
  callbackList["fetch-sync-devices"](null);
});

chrome.onevnSync.onResolveSyncRecords.addListener(function(category_name, recordsAndExistingObjects) {
...
chrome.onevnSync.onSendSyncRecords.addListener(function(category_name, records) {
  // Fixup ids
...
chrome.onevnSync.onSendGetBookmarksBaseOrder.addListener(function(deviceId, platform) {
...
chrome.onevnSync.onNeedSyncWords.addListener(function(seed) {
  ...
  chrome.onevnSync.syncWordsPrepared(words);
});

chrome.onevnSync.onLoadClient.addListener(function() {
  console.log("in chrome.onevnSync.onLoadClient");
  LoadJsLibScript();
});

chrome.onevnSync.onClearOrderMap.addListener(function() {
  orderMap = {};
});

chrome.onevnSync.extensionInitialized();
console.log("chrome.onevnSync.extensionInitialized");
    ...
    if (!record.bookmark.order) {
      getBookmarkOrderCallback = (order) => {
        ...
        if (record.objectIdStr)
          chrome.onevnSync.saveBookmarkOrder(record.objectIdStr, order);
  ...
  handleMessage(message, arg1, arg2, arg3, arg4) {
    switch (message) {
      case "get-init-data":
        ...
        chrome.onevnSync.getInitData(arg1/*syncVersion*/);
        break;
      case "sync-setup-error":
        ...
        chrome.onevnSync.syncSetupError(errorToPass);
        break;
      case "sync-debug":
        console.log(`"sync-debug" message=${JSON.stringify(arg1)}`);
        chrome.onevnSync.syncDebug(arg1/*message*/);
        break;
      case "save-init-data":
        var deviceId = arg2;
        ...
        chrome.onevnSync.saveInitData(arg1/*seed*/, deviceId);
        break;
      case "sync-ready":
        console.log(`"sync-ready"`);
        chrome.onevnSync.syncReady();
        break;
      case "get-existing-objects":
        ...
        chrome.onevnSync.getExistingObjects(arg1/*category_name*/,
      ...
      case "resolved-sync-records":
        ...
        chrome.onevnSync.resolvedSyncRecords(arg1/*categoryName*/, arg2/*records*/);
        break;
      case "save-bookmarks-base-order":
        console.log(`"save-bookmarks-base-order" order=${JSON.stringify(arg1)} `);
        chrome.onevnSync.saveBookmarksBaseOrder(arg1/*order*/);
        break;
...
function LoadJsLibScript() {
  var sync_bundle_url = chrome.runtime.getURL('extension/onevn-sync/bundles/bundle.js');
