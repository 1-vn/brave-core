  ...
  switch (action.type) {
    ...
    case types.GET_GRANT_CAPTCHA:
      ...
      if (currentGrant.promotionId && currentGrant.type) {
        chrome.onevnRewards.getGrantCaptcha(currentGrant.promotionId, currentGrant.type)
      }
      break
    ...
    case types.SOLVE_GRANT_CAPTCHA: {
      ...
      if (promotionId && action.payload.x && action.payload.y) {
        chrome.onevnRewards.solveGrantCaptcha(JSON.stringify({
          x: action.payload.x,
          y: action.payload.y
        }), promotionId)
      }
