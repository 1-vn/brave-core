...
export const rewardsPanelReducer = (state: RewardsExtension.State | undefined, action: any) => {
  ...
  switch (action.type) {
    case types.CREATE_WALLET:
      chrome.onevnRewards.createWallet()
      state = { ...state }
    ...
    case types.ON_WALLET_INITIALIZED: {
      ...
      if (result === RewardsExtension.Result.WALLET_CREATED) {
        ...
        state.walletCorrupted = false
        chrome.onevnRewards.saveAdsSetting('adsEnabled', 'true')
    ...
    case types.ON_TAB_RETRIEVED: {
      ...
      if (!publisher || (publisher && (publisher.tabUrl !== tab.url || !validKey))) {
        chrome.onevnRewards.getPublisherData(
          tab.windowId,
          tab.url,
          tab.favIconUrl || '',
    ...
    case types.GET_WALLET_PROPERTIES:
      chrome.onevnRewards.getWalletProperties()
      break
    ...
    case types.GET_CURRENT_REPORT:
      chrome.onevnRewards.getCurrentReport()
      break
    ...
    case types.INCLUDE_IN_AUTO_CONTRIBUTION: {
      ...
      chrome.onevnRewards.includeInAutoContribution(publisherKey, excluded)
      break
    }
    ...
    case types.ON_SETTING_SAVE: {
      ...
      if (key) {
        state[key] = value
        chrome.onevnRewards.saveSetting(key, value)
      }
      break
    }
    case types.REMOVE_RECURRING_TIP: {
      ...
      chrome.onevnRewards.removeRecurringTip(publisherKey)
      break
    }
    case types.SAVE_RECURRING_TIP: {
      ...
      chrome.onevnRewards.saveRecurringTip(publisherKey, newAmount)
      break
    }
