...
import { BatColorIcon, WalletAddIcon } from 'onevn-ui/components/icons'
...
export class RewardsPanel extends React.Component<Props, State> {
  ...
  componentDidMount () {
    ...
    chrome.onevnRewards.getRewardsMainEnabled(((enabled: boolean) => {
      this.props.actions.onEnabledMain(enabled)
    }))

    chrome.onevnRewards.getACEnabled(((enabled: boolean) => {
  ...
  openPrivacyPolicy () {
    chrome.tabs.create({
      url: 'https://1-vn.com/privacy#rewards'
    })
  }
  ...
  openTOS () {
    chrome.tabs.create({
      url: 'https://1-vn.com/terms-of-use'
    })
  }
