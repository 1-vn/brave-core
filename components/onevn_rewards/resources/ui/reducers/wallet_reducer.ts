...
const createWallet = (state: Rewards.State) => {
  ...
  chrome.send('onevn_rewards.getReconcileStamp')
  chrome.send('onevn_rewards.getAddresses')
...
const walletReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    case types.CREATE_WALLET:
      ...
      chrome.send('onevn_rewards.createWalletRequested')
      break
    case types.WALLET_CREATED:
      state = { ...state }
      state = createWallet(state)
      chrome.send('onevn_rewards.saveAdsSetting', ['adsEnabled', 'true'])
      break
    ...
    case types.GET_WALLET_PROPERTIES:
      chrome.send('onevn_rewards.getWalletProperties')
      break
    ...
    case types.GET_WALLLET_PASSPHRASE:
      chrome.send('onevn_rewards.getWalletPassphrase')
      break
    ...
    case types.RECOVER_WALLET: {
      ...
      chrome.send('onevn_rewards.recoverWallet', [key])
      break
    }
    ...
    case types.GET_CURRENT_REPORT: {
      chrome.send('onevn_rewards.getBalanceReports')
      break
    }
    ...
    case types.CHECK_WALLET_EXISTENCE: {
      chrome.send('onevn_rewards.checkWalletExistence')
      break
    }
    ...
    case types.GET_PENDING_CONTRIBUTION_TOTAL: {
      chrome.send('onevn_rewards.getPendingContributionsTotal')
      break
    }
    ...
    case types.GET_ADDRESSES_FOR_PAYMENT_ID: {
      chrome.send('onevn_rewards.getAddressesForPaymentId')
      break
    }
