...
const publishersReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    ...
    case types.ON_EXCLUDE_PUBLISHER: {
      ...
      chrome.send('onevn_rewards.excludePublisher', [publisherKey])
      break
    }
    case types.ON_RESTORE_PUBLISHERS:
      state = { ...state }
      chrome.send('onevn_rewards.restorePublishers', [])
      break
    ...
    case types.REMOVE_RECURRING_TIP:
      ...
      chrome.send('onevn_rewards.removeRecurringTip', [action.payload.publisherKey])
      break
    ...
    case types.GET_EXCLUDED_PUBLISHERS_NUMBER:
      chrome.send('onevn_rewards.getExcludedPublishersNumber')
      break
