...
const rewardsReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    ...
    case types.ON_SETTING_SAVE:
      ...
      if (key) {
        state[key] = value
        chrome.send('onevn_rewards.saveSetting', [key, value.toString()])
      }
      break
    ...
    case types.GET_DONATION_TABLE: {
      chrome.send('onevn_rewards.getRecurringTips')
      chrome.send('onevn_rewards.getOneTimeTips')
      break
    }
    case types.GET_CONTRIBUTE_LIST: {
      chrome.send('onevn_rewards.getContributionList')
      break
    }
    case types.CHECK_IMPORTED: {
      chrome.send('onevn_rewards.checkImported')
      break
    }
    ...
    case types.GET_ADS_DATA: {
      chrome.send('onevn_rewards.getAdsData')
      break
    }
    ...
    case types.ON_ADS_SETTING_SAVE: {
      ...
      if (key) {
        state[key] = value
        chrome.send('onevn_rewards.saveAdsSetting', [key, value.toString()])
      }
      break
    }
    ...
    case types.ON_TRANSACTION_HISTORY_FOR_THIS_CYCLE_CHANGED: {
      chrome.send('onevn_rewards.getTransactionHistoryForThisCycle', [])
      break
    }
    ...
    case types.GET_REWARDS_MAIN_ENABLED: {
      chrome.send('onevn_rewards.getRewardsMainEnabled', [])
      break
    }
