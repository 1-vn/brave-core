...
const grantReducer: Reducer<Rewards.State | undefined> = (state: Rewards.State, action) => {
  switch (action.type) {
    case types.GET_GRANTS:
      chrome.send('onevn_rewards.getGrants', [])
      break
    case types.ON_GRANT:
      state = { ...state }
    ...
    case types.GET_GRANT_CAPTCHA:
      ...
      if (currentGrant.promotionId && currentGrant.type) {
        chrome.send('onevn_rewards.getGrantCaptcha', [currentGrant.promotionId, currentGrant.type])
      }
      break
    ...
    case types.SOLVE_GRANT_CAPTCHA: {
      ...
      if (promotionId && action.payload.x && action.payload.y) {
        chrome.send('onevn_rewards.solveGrantCaptcha', [JSON.stringify({
          x: action.payload.x,
          y: action.payload.y
        }), promotionId])
      }
      ...
      switch (properties.status) {
        ...
        case 6:
          ...
          if (state.currentGrant.promotionId && state.currentGrant.type) {
            chrome.send('onevn_rewards.getGrantCaptcha', [state.currentGrant.promotionId, state.currentGrant.type])
          }
          break
        case 13:
