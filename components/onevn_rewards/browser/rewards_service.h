...
#ifndef ONEVN_COMPONENTS_ONEVN_REWARDS_BROWSER_REWARDS_SERVICE_H_
#define ONEVN_COMPONENTS_ONEVN_REWARDS_BROWSER_REWARDS_SERVICE_H_
...
#include "onevn/components/onevn_rewards/browser/rewards_internals_info.h"
...
using GetAllBalanceReportsCallback = base::Callback<void(
    const std::map<std::string, onevn_rewards::BalanceReport>&)>;
...
using GetAutoContributePropsCallback = base::Callback<void(
    std::unique_ptr<onevn_rewards::AutoContributeProps>)>;
...
using GetRewardsInternalsInfoCallback = base::OnceCallback<void(
    std::unique_ptr<onevn_rewards::RewardsInternalsInfo>)>;
...
using GetPublisherBannerCallback =
    base::OnceCallback<void(std::unique_ptr<onevn_rewards::PublisherBanner>)>;
...
class RewardsService : public KeyedService {
 public:
  ...
  virtual void OnDonate(const std::string& publisher_key, int amount,
      bool recurring, std::unique_ptr<onevn_rewards::ContentSite> site) = 0;
...
#endif  // ONEVN_COMPONENTS_ONEVN_REWARDS_BROWSER_REWARDS_SERVICE_H_
