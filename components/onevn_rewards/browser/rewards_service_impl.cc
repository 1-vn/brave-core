...
#include "onevn/browser/ui/webui/onevn_rewards_source.h"
...
#include "onevn/components/onevn_rewards/browser/auto_contribution_props.h"
...
#include "onevn/components/onevn_rewards/browser/publisher_info_database.h"
#include "onevn/components/onevn_rewards/browser/rewards_fetcher_service_observer.h"
...
#include "onevn/components/onevn_rewards/browser/switches.h"
#include "onevn/components/onevn_rewards/browser/wallet_properties.h"
#include "onevn/components/services/bat_ledger/public/cpp/ledger_client_mojo_proxy.h"
...
void RewardsServiceImpl::CreateWallet() {
  if (ready().is_signaled()) {
    ...
    ready().Post(FROM_HERE,
        base::Bind(&onevn_rewards::RewardsService::CreateWallet,
...
void RewardsServiceImpl::OnWalletProperties(
  ...
  for (auto& observer : observers_) {
    if (wallet_info) {
      wallet_properties.reset(new onevn_rewards::WalletProperties);
...
void RewardsServiceImpl::OnGetAutoContributeProps(
  ...
  auto auto_contri_props =
    std::make_unique<onevn_rewards::AutoContributeProps>();
...
void RewardsServiceImpl::OnGetRewardsInternalsInfo(
  ...
  auto rewards_internals_info =
      std::make_unique<onevn_rewards::RewardsInternalsInfo>();
...
void RewardsServiceImpl::LoadNicewareList(
  ...
  std::string data = ui::ResourceBundle::GetSharedInstance().GetRawDataResource(
      IDR_ONEVN_REWARDS_NICEWARE_LIST).as_string();
...
void RewardsServiceImpl::FetchWalletProperties() {
  if (ready().is_signaled()) {
    ...
    ready().Post(FROM_HERE,
        base::Bind(&onevn_rewards::RewardsService::FetchWalletProperties,
            base::Unretained(this)));
  }
}
...
void RewardsServiceImpl::TriggerOnRecoverWallet(ledger::Result result,
                                                double balance,
                                    const std::vector<ledger::Grant>& grants) {
  std::vector<onevn_rewards::Grant> newGrants;
...
void RewardsServiceImpl::SetConfirmationsIsReady(const bool is_ready) {
  auto* ads_service = onevn_ads::AdsServiceFactory::GetForProfile(profile_);
  if (ads_service)
...
// TODO(onevn): Remove me (and pure virtual definition)
// see https://github.com/1-vn/onevn-core/commit/c4ef62c954a64fca18ae83ff8ffd611137323420#diff-aa3505dbf36b5d03d8ba0751e0c99904R385
// and https://github.com/1-vn-intl/bat-native-ledger/commit/27f3ceb471d61c84052737ff201fe18cb9a6af32#diff-e303122e010480b2226895b9470891a3R135
...
void RewardsServiceImpl::OnGetAllBalanceReports(
  ...
  std::map<std::string, onevn_rewards::BalanceReport> newReports;
  for (auto const& report : reports) {
    onevn_rewards::BalanceReport newReport;
...
void RewardsServiceImpl::OnPublisherBanner(
  ...
  std::unique_ptr<onevn_rewards::PublisherBanner> new_banner;
  new_banner.reset(new onevn_rewards::PublisherBanner());
...
bool SaveContributionInfoOnFileTaskRunner(
    const onevn_rewards::ContributionInfo info,
  PublisherInfoDatabase* backend) {
...
void RewardsServiceImpl::SaveContributionInfo(const std::string& probi,
  ...
  const std::string& publisher_key,
  const ledger::REWARDS_CATEGORY category) {
  onevn_rewards::ContributionInfo info;
  info.probi = probi;
  info.month = month;
  info.year = year;
  info.date = date;
...
bool SaveRecurringTipOnFileTaskRunner(
    const onevn_rewards::RecurringDonation info,
  PublisherInfoDatabase* backend) {
...
void RewardsServiceImpl::SaveRecurringTip(
  ...
  onevn_rewards::RecurringDonation info;
  info.publisher_key = publisher_key;
  info.amount = amount;
...
void RewardsServiceImpl::OnGetRecurringTipsUI(
    ...
    const std::vector<std::string>& json_list) {
    std::unique_ptr<onevn_rewards::ContentSiteList> new_list(
      new onevn_rewards::ContentSiteList);

  for (auto &json_publisher : json_list) {
    ledger::PublisherInfo publisher;
    publisher.loadFromJson(json_publisher);
    onevn_rewards::ContentSite site = PublisherInfoToContentSite(publisher);
...
void RewardsServiceImpl::TriggerOnGetCurrentBalanceReport(
  ...
  for (auto& observer : private_observers_) {
    onevn_rewards::BalanceReport balance_report;
...
void RewardsServiceImpl::OnDonate(
    ...
    int amount,
    bool recurring,
    std::unique_ptr<onevn_rewards::ContentSite> site) {
