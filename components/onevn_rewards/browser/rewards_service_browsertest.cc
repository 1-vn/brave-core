...
using onevnledger_bat_helper::SERVER_TYPES;
...
bool URLMatches(const std::string& url,
  ...
  const std::string target_url =
      onevnledger_bat_helper::buildURL(path, prefix, server);
...
void OneVNURLFetcher::DetermineURLResponsePath(const std::string& url) {
  ...
  if (url.find(onevnledger_bat_helper::buildURL(REGISTER_PERSONA, PREFIX_V2,
    onevnledger_bat_helper::SERVER_TYPES::LEDGER)) == 0
    && tmp.size() == 6) {
    SetResponseString(onevn_test_resp::registrarVK_);
  } else if (URLMatches(url, REGISTER_PERSONA, PREFIX_V2,
                        SERVER_TYPES::LEDGER) &&
             tmp.size() == 7) {
    SetResponseString(onevn_test_resp::verification_);
  } else if (URLMatches(url, WALLET_PROPERTIES, PREFIX_V2,
                        SERVER_TYPES::BALANCE)) {
    SetResponseString(onevn_test_resp::wallet_);
  } else if (URLMatches(url, WALLET_PROPERTIES, PREFIX_V2,
                        SERVER_TYPES::LEDGER)) {
    GURL gurl(url);
    if (gurl.has_query())
      SetResponseString(onevn_test_resp::reconcile_);
    else
      SetResponseString(onevn_test_resp::current_reconcile_);
  } else if (URLMatches(url, GET_SET_PROMOTION, PREFIX_V2,
                        SERVER_TYPES::LEDGER)) {
    GURL gurl(url);
    if (gurl.has_query())
      SetResponseString(onevn_test_resp::grant_);
    else
      SetResponseString(onevn_test_resp::captcha_solution_);
  } else if (URLMatches(url, GET_SET_PROMOTION, PREFIX_V4,
                        SERVER_TYPES::LEDGER)) {
    SetResponseString(onevn_test_resp::grant_v4_);
    ...
    SetResponseString(onevn_test_resp::captcha_);
  } else if (URLMatches(url, RECONCILE_CONTRIBUTION, PREFIX_V2,
                        SERVER_TYPES::LEDGER)) {
    SetResponseString(onevn_test_resp::contribution_);
  } else if (URLMatches(url, REGISTER_VIEWING, PREFIX_V2,
                        SERVER_TYPES::LEDGER)) {
    if (url.find(REGISTER_VIEWING "/") != std::string::npos)
      SetResponseString(onevn_test_resp::register_credential_);
    else
      SetResponseString(onevn_test_resp::register_);
  } else if (URLMatches(url, SURVEYOR_BATCH_VOTING, PREFIX_V2,
                        SERVER_TYPES::LEDGER)) {
    if (url.find(SURVEYOR_BATCH_VOTING "/") != std::string::npos)
      SetResponseString(onevn_test_resp::surveyor_voting_credential_);
    else
      SetResponseString(onevn_test_resp::surveyor_voting_);
...
class OneVNRewardsBrowserTest : public InProcessBrowserTest,
 ...
 public:
  void SetUpOnMainThread() override {
    ...
    ReadTestData();
    rewards_service_ = static_cast<onevn_rewards::RewardsServiceImpl*>(
        onevn_rewards::RewardsServiceFactory::GetForProfile(
            browser()->profile()));
  ...
  void GetTestDataDir(base::FilePath* test_data_dir) {
    ...
    ASSERT_TRUE(base::PathService::Get(onevn::DIR_TEST_DATA, test_data_dir));
  ...
  GURL rewards_url() {
    GURL rewards_url("onevn://rewards");
    return rewards_url;
  }
  ...
  onevn_rewards::RewardsServiceImpl* rewards_service() {
  ...
  void OnPublisherListNormalized(onevn_rewards::RewardsService* rewards_service,
  ...
  onevn_rewards::RewardsServiceImpl* rewards_service_;
