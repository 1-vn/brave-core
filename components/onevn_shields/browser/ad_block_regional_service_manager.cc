...
#include "onevn/components/onevn_shields/browser/ad_block_service_helper.h"
#include "onevn/vendor/ad-block/lists/regions.h"
...
void AdBlockRegionalServiceManager::StartRegionalServices() {
  ...
  if (!checked_default_region) {
    ...
    auto it = onevn_shields::FindAdBlockFilterListByLocale(
        region_lists, g_onevn_browser_process->GetApplicationLocale());
...
bool AdBlockRegionalServiceManager::IsSupportedLocale(
  ...
  return (onevn_shields::FindAdBlockFilterListByLocale(region_lists, locale) !=
          region_lists.end());
}
