...
#include "onevn/components/onevn_shields/browser/ad_block_regional_service.h"
...
class AdBlockServiceTest : public ExtensionBrowserTest {
 public:
  ...
  void PreRunTestOnMainThread() override {
    ...
    ASSERT_TRUE(g_onevn_browser_process->ad_block_service()->IsInitialized());
  }

  void AddRulesToAdBlock(const char* rules) {
    g_onevn_browser_process->ad_block_service()
        ->GetAdBlockClientForTest()
        ->parse(rules);
  }

  void AssertTagExists(const std::string& tag, bool expected_exists) const {
    bool exists_default = g_onevn_browser_process->ad_block_service()
                              ->GetAdBlockClientForTest()
                              ->tagExists(tag);
  ...
  void SetDefaultComponentIdAndBase64PublicKeyForTest(
    ...
    onevn_shields::AdBlockService::SetComponentIdAndBase64PublicKeyForTest(
  ...
  void SetRegionalComponentIdAndBase64PublicKeyForTest(
      const std::string& component_id,
      const std::string& component_base64_public_key) {
    onevn_shields::AdBlockRegionalService::
  ...
  void SetDATFileVersionForTest(const std::string& dat_file_version) {
    onevn_shields::AdBlockService::SetDATFileVersionForTest(dat_file_version);
  }
  ...
  bool InstallDefaultAdBlockExtension(
    ...
    g_onevn_browser_process->ad_block_service()->OnComponentReady(
  ...
  bool InstallRegionalAdBlockExtension(const std::string& uuid) {
    ...
    EXPECT_EQ(g_onevn_browser_process->ad_block_regional_service_manager()
  ...
  bool InstallTrackingProtectionExtension() {
    ...
    g_onevn_browser_process->tracking_protection_service()->OnComponentReady(
  ...
  bool StartAdBlockRegionalServices() {
    ...
    if (!g_onevn_browser_process->ad_block_regional_service_manager()
             ->IsInitialized())
      return false;
    return true;
  }
  ...
  void WaitForDefaultAdBlockServiceThread() {
    scoped_refptr<base::ThreadTestHelper> io_helper(new base::ThreadTestHelper(
        g_onevn_browser_process->ad_block_service()->GetTaskRunner()));
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest,
  ...
  ASSERT_TRUE(g_onevn_browser_process->ad_block_custom_filters_service()
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, SocialButttonAdBLockTagTest) {
  AddRulesToAdBlock(base::StringPrintf("||example.com^$tag=%s",
                    onevn_shields::kFacebookEmbeds).c_str());
  EXPECT_EQ(browser()->profile()->GetPrefs()->GetUint64(kAdsBlocked), 0ULL);
  GURL tab_url = embedded_test_server()->GetURL("b.com",
                                                kAdBlockTestPage);
  g_onevn_browser_process->ad_block_service()->EnableTag(
      onevn_shields::kFacebookEmbeds, true);
  WaitForDefaultAdBlockServiceThread();
  GURL resource_url =
...
IN_PROC_BROWSER_TEST_F(AdBlockServiceTest, TagPrefsControlTags) {
  ...
  AssertTagExists(onevn_shields::kFacebookEmbeds, true);
  AssertTagExists(onevn_shields::kTwitterEmbeds, true);
  AssertTagExists(onevn_shields::kLinkedInEmbeds, false);
  ...
  AssertTagExists(onevn_shields::kFacebookEmbeds, true);
  AssertTagExists(onevn_shields::kTwitterEmbeds, true);
  AssertTagExists(onevn_shields::kLinkedInEmbeds, true);

  prefs->SetBoolean(kFBEmbedControlType, false);
  WaitForAllAdBlockServiceThreads();
  AssertTagExists(onevn_shields::kFacebookEmbeds, false);
  AssertTagExists(onevn_shields::kTwitterEmbeds, true);
  AssertTagExists(onevn_shields::kLinkedInEmbeds, true);
  ...
  WaitForAllAdBlockServiceThreads();
  AssertTagExists(onevn_shields::kFacebookEmbeds, false);
  AssertTagExists(onevn_shields::kTwitterEmbeds, false);
  AssertTagExists(onevn_shields::kLinkedInEmbeds, true);
