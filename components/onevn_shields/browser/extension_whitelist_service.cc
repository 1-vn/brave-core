...
#include "onevn/vendor/extension-whitelist/extension_whitelist_parser.h"
...
// The extension whitelist factory. Using the Onevn Shields as a singleton
...
std::unique_ptr<ExtensionWhitelistService> ExtensionWhitelistServiceFactory() {
  ...
  g_onevn_browser_process->local_data_files_service()->AddObserver(
    service.get());
  return service;
}
