...
bool GetDefaultFromResourceIdentifier(const std::string& resource_identifier,
  ...
  if (resource_identifier == onevn_shields::kAds) {
    return false;
  } else if (resource_identifier == onevn_shields::kTrackers) {
    return false;
    ...
    return false;
  } else if (resource_identifier == onevn_shields::kCookies) {
...
bool ShouldSetReferrer(bool allow_referrers,
  ...
  if (!output_referrer || allow_referrers || !shields_up ||
      ...
      (g_onevn_browser_process &&
       g_onevn_browser_process->referrer_whitelist_service()->IsWhitelisted(
