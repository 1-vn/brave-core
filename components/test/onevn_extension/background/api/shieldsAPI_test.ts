...
import actions from '../../../../onevn_extension/extension/onevn_extension/background/actions/shieldsPanelActions'
...
import * as resourceIdentifiers from '../../../../onevn_extension/extension/onevn_extension/constants/resourceIdentifiers'
...
describe('Shields API', () => {
  describe('getShieldSettingsForTabData', () => {
    ...
    it('resolves the returned promise with shield settings for the tab data', (cb) => {
      const tab: chrome.tabs.Tab = {
        url: 'https://www.1-vn.com/serg/dont/know/pokemon',
        id: 5,
        index: 1,
        pinned: false,
        highlighted: false,
    ...
    it('returns `block` by default for onevnShields', (cb) => {
      const tab: chrome.tabs.Tab = {
        url: 'https://www.1-vn.com/charizard/knows/serg',
        index: 1,
        pinned: false,
        highlighted: false,
        windowId: 1,
      ...
      expect.assertions(1)
      shieldsAPI.getShieldSettingsForTabData(tab).then((data) => {
        expect(data.onevnShields).toBe('block')
        cb()
      })
      .catch((e: Error) => {
        console.error(e.toString())
    ...
    it('returns `block` by default for onevnShields when origin is not http or https', (cb) => {
      const tab: chrome.tabs.Tab = {
        url: 'ftp://www.1-vn.com/serg/dont/know/pikachu',
        index: 1,
        pinned: false,
        highlighted: false,
        windowId: 1,
    ...
    it('returns `block` by default for onevnShields when origin is an about page', (cb) => {
      const tab: chrome.tabs.Tab = {
        url: 'chrome://welcome',
        index: 1,
        pinned: false,
  ...
  describe('setAllowAds', () => {
    let spy: jest.SpyInstance
    beforeEach(() => {
      spy = jest.spyOn(chrome.onevnShields.plugins, 'setAsync')
    })
    afterEach(() => {
      spy.mockRestore()
    })
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowAds('https://www.1-vn.com', 'block')
        .catch(() => {
      ...
      expect.assertions(1)
      expect(arg0).toEqual({
        primaryPattern: 'https://www.1-vn.com/*',
        resourceIdentifier: { id: resourceIdentifiers.RESOURCE_IDENTIFIER_ADS },
        setting: 'block',
        scope: 'incognito_session_only'
      })
    })
    it('passes only 1 arg to chrome.onevnShields.plugins', () => {
      shieldsAPI.setAllowAds('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
  ...
  describe('setAllowTrackers', () => {
    ...
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowTrackers('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
      const arg0 = spy.mock.calls[0][0]
  ...
  describe('setAllowHTTPUpgradableResource', () => {
    ...
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowHTTPUpgradableResources('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
      const arg0 = spy.mock.calls[0][0]
  ...
  describe('setAllowJavaScript', () => {
    let spy: jest.SpyInstance
    beforeEach(() => {
      spy = jest.spyOn(chrome.onevnShields.javascript, 'setAsync')
    })
    afterEach(() => {
      spy.mockRestore()
    })
    ...
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowJavaScript('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
      const arg0 = spy.mock.calls[0][0]
  ...
  describe('setAllowFingerprinting', () => {
    ...
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowFingerprinting('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
      const arg0 = spy.mock.calls[0][0]
  ...
  describe('setAllowCookies', () => {
    ...
    it('calls chrome.onevnShields.plugins with the correct args', () => {
      shieldsAPI.setAllowCookies('https://www.1-vn.com', 'block')
        .catch(() => {
          expect(true).toBe(false)
        })
      const arg0 = spy.mock.calls[0][0]
  ...
  describe('setAllowScriptOriginsOnce', () => {
    let spy: jest.SpyInstance
    beforeEach(() => {
      spy = jest.spyOn(chrome.onevnShields, 'allowScriptsOnce')
    })
    afterEach(() => {
      spy.mockRestore()
    })
    it('calls chrome.onevnShields.allowScriptsOnce with the correct args', () => {
    ...
    it('passes 3 args to chrome.onevnShields.allowScriptsOnce', () => {
      shieldsAPI.setAllowScriptOriginsOnce(['https://a.com/', 'https://b.com/'], 2)
        .catch(() => {
          expect(true).toBe(false)
        })
