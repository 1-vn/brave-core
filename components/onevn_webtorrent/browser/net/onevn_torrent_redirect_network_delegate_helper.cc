...
bool IsWebTorrentDisabled(net::URLRequest* request) {
  ...
  return !infoMap->extensions().Contains(onevn_webtorrent_extension_id) ||
    infoMap->disabled_extensions().Contains(onevn_webtorrent_extension_id);
}
...
int OnHeadersReceived_TorrentRedirectWork(
  ...
  *override_response_headers =
  ...
  GURL url(
      base::StrCat({extensions::kExtensionScheme, "://",
      onevn_webtorrent_extension_id,
      "/extension/onevn_webtorrent.html?",
      request->url().spec()}));
