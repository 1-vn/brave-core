...
void OneVNImporter::ImportBookmarks() {
  ...
  if (!bookmarks.empty() && !cancelled()) {
    const base::string16& first_folder_name =
      bridge_->GetLocalizedString(IDS_BOOKMARK_GROUP_FROM_ONEVN);
...
bool ParsePaymentsPreferences(OneVNLedger* ledger,
  ...
  // Contribution amount
  // TODO(onevn): Get default amount from rewards service
  ...
  switch (payments->min_visit_time) {
    ...
    default:
      payments->min_visit_time =
        onevnledger_ledger::_default_min_publisher_duration;
  }
...
std::vector<ImportedBrowserTab> ParseTabs(const base::Value* frames) {
  ...
  for (const auto& frame : frames->GetList()) {
    ...
    // Filter internal URLs from Muon that won't resolve correctly in onevn-core
    if (!CanImportURL(url))
      continue;
...
void OneVNImporter::ImportWindows() {
  ...
  // onevn-core. To manage this transition, import all pinned tabs into the
