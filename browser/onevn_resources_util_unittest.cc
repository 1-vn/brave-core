...
#include "onevn/grit/onevn_theme_resources.h"
...
TEST(OnevnResourcesUtil, CheckIds) {
  const struct {
    ...
    // IDRs from onevn/app/theme/onevn_theme_resources.grd should be valid.
...
TEST(OnevnResourcesWinTest, CheckStringsForInstaller) {
  ...
  // with onevn's one.
  // onevn::InitializeResourceBundle can only be called once per unittest
