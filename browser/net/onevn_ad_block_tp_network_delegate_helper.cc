...
std::string GetGoogleTagManagerPolyfillJS() {
  ...
  std::string str = ui::ResourceBundle::GetSharedInstance().GetRawDataResource(
      IDR_ONEVN_TAG_MANAGER_POLYFILL).as_string();
...
std::string GetGoogleTagServicesPolyfillJS() {
  ...
  std::string str = ui::ResourceBundle::GetSharedInstance().GetRawDataResource(
    IDR_ONEVN_TAG_SERVICES_POLYFILL).as_string();
...
bool GetPolyfillForAdBlock(bool allow_onevn_shields, bool allow_ads,
  ...
  // are up.
  if (!allow_onevn_shields || allow_ads) {
    return false;
  }
...
void OnBeforeURLRequestAdBlockTPOnTaskRunner(
  ...
  if (!g_onevn_browser_process->ad_block_service()->ShouldStartRequest(
...
int OnBeforeURLRequest_AdBlockTPPreWork(
  ...
  if (GetPolyfillForAdBlock(ctx->allow_onevn_shields, ctx->allow_ads,
  ...
  // If the following info isn't available, then proper content settings can't
  // be looked up, so do nothing.
  if (ctx->tab_origin.is_empty() || !ctx->allow_onevn_shields ||
      ctx->allow_ads ||
