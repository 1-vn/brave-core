...
#include "onevn/browser/ui/views/importer/onevn_import_lock_dialog_view.h"
...
ImportLockDialogView::ImportLockDialogView(
  ...
  } else {  // if (source_profile_.importer_type == ::importer::TYPE_ONEVN)
    description_label =
      new views::Label(l10n_util::GetStringUTF16(IDS_ONEVN_IMPORTER_LOCK_TEXT));
  }
...
base::string16 ImportLockDialogView::GetWindowTitle() const {
  if (source_profile_.importer_type == ::importer::TYPE_CHROME) {
    ...
    return l10n_util::GetStringUTF16(IDS_ONEVN_IMPORTER_LOCK_TITLE);
  }
}
