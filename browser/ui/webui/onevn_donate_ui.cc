...
#include "onevn/browser/ui/webui/onevn_donate_ui.h"
...
#include "onevn/components/onevn_rewards/resources/grit/onevn_donate_generated_map.h"
...
void RewardsDonateDOMHandler::RegisterMessages() {
  web_ui()->RegisterMessageCallback(
      "onevn_rewards_donate.getPublisherBanner",
      base::BindRepeating(&RewardsDonateDOMHandler::GetPublisherDonateData,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "onevn_rewards_donate.getWalletProperties",
      base::BindRepeating(&RewardsDonateDOMHandler::GetWalletProperties,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "onevn_rewards_donate.onDonate",
       base::BindRepeating(&RewardsDonateDOMHandler::OnDonate,
                           base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "onevn_rewards_donate.getRecurringTips",
      base::BindRepeating(&RewardsDonateDOMHandler::GetRecurringTips,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "onevn_rewards_donate.getReconcileStamp",
...
void RewardsDonateDOMHandler::OnWalletProperties(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "onevn_rewards_donate.walletProperties", result);
}
...
void RewardsDonateDOMHandler::GetRecurringTips(
    const base::ListValue *args) {
  if (rewards_service_) {
    rewards_service_->GetRecurringTipsUI(base::BindOnce(
          &RewardsDonateDOMHandler::OnGetRecurringTips,
          weak_factory_.GetWeakPtr()));
  }
}

void RewardsDonateDOMHandler::OnGetRecurringTips(
    std::unique_ptr<onevn_rewards::ContentSiteList> list) {
  if (!web_ui()->CanCallJavascript()) {
    return;
  }
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "onevn_rewards_donate.recurringTips", *publishers);
}
...
void RewardsDonateDOMHandler::OnPublisherBanner(
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "onevn_rewards_donate.publisherBanner", result);
}
...
void RewardsDonateDOMHandler::OnReconcileStamp(uint64_t reconcile_stamp) {
  ...
  web_ui()->CallJavascriptFunctionUnsafe("onevn_rewards_donate.reconcileStamp",
      base::Value(stamp));
}

void RewardsDonateDOMHandler::OnRecurringTipRemoved(
    onevn_rewards::RewardsService* rewards_service,
    bool success) {
  if (!web_ui()->CanCallJavascript()) {
     return;
  ...
  web_ui()->CallJavascriptFunctionUnsafe(
      "onevn_rewards_donate.recurringTipRemoved", base::Value(success));
}
