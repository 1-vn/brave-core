...
#include "onevn/components/onevn_new_tab/resources/grit/onevn_new_tab_generated_map.h"
...
class NewTabDOMHandler : public content::WebUIMessageHandler {
 ...
 private:
  ...
  void HandleToggleAlternativeSearchEngineProvider(
    ...
    onevn::ToggleUseAlternativeSearchEngineProvider(
...
void OnevnNewTabUI::CustomizeNewTabWebUIProperties(content::RenderViewHost* render_view_host) {
  ...
  if (render_view_host) {
    ...
    render_view_host->SetWebUIProperty(
        "isQwant", onevn::IsRegionForQwant(profile) ? "true" : "false");
  }
}
