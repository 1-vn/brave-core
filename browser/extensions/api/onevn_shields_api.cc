...
OnevnShieldsContentSettingSetFunction::Run() {
  ...
  if (params->details.scope ==
      onevn_shields::SCOPE_INCOGNITO_SESSION_ONLY) {
  ...
  if (incognito) {
    ...
    if (!browser_context()->IsOffTheRecord() &&
        !extensions::util::IsIncognitoEnabled(onevn_extension_id,
                                              browser_context())) {
      return RespondNow(Error(pref_keys::kIncognitoErrorMessage));
    }
  } else {
  ...
  // Delete previous settings set by onevn extension in extension's
